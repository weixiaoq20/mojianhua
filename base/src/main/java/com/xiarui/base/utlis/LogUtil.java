...
public final class LogUtil {
    ...
    public static class Builder {
        ...
        public Builder() {
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
                dir = AppManagerUtil.getInstance().getApplication()
                        .getExternalCacheDir() + File.separator + "log" + File.separator;
            } else {
                dir = AppManagerUtil.getInstance().getApplication()
                        .getCacheDir() + File.separator + "log" + File.separator;
            }
        }
    ...
    private static void printBorder(int type, String tag, boolean isTop) {
        ...
        switch (type) {
            case V:
                android.util.Log.v(tag, border);
                break;
            case D:
                android.util.Log.d(tag, border);
                break;
            case I:
                android.util.Log.i(tag, border);
                break;
            case W:
                android.util.Log.w(tag, border);
                break;
            case E:
                android.util.Log.e(tag, border);
                break;
            case A:
                android.util.Log.wtf(tag, border);
                break;
        }
    }
    ...
    private synchronized static void print2File(final String tag, final String msg) {
        Date now = new Date();
        String date = new SimpleDateFormat("MM-dd", java.util.Locale.getDefault()).format(now);
        final String fullPath = dir + date + ".txt";
        if (!createOrExistsFile(fullPath)) {
            android.util.Log.e(tag, "log to " + fullPath + " failed!");
            return;
        }
        String time = new SimpleDateFormat("MM-dd HH:mm:ss.SSS ", java.util.Locale.getDefault()).format(now);
        StringBuilder sb = new StringBuilder();
